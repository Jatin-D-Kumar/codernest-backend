...
import { SSOError } from "../utils/SSOError.js";
...
try {
  ...
  passport.deserializeUser(async (id, next) => {
    try {
      ...
      else next(new SSOError(404, "User does not exist"), null); // throw an error if user does not exist
    } catch (error) {
      next(
        new SSOError(
          500,
      ...
      async (_, __, profile, next) => {
        ...
        if (user) {
          ...
          if (createdUser) {
            ...
            next(new SSOError(500, "Error while registering the user"), null);
          }
        }
      }
    )
          ...
          if (!profile._json.email) {
            ...
            const createdUser = await User.create({
              ...
              username: profile?.username
                ? profile?.username // if username already exist, set the emails first half as the username
